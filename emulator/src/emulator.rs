...
impl Emulator {
    ...
    pub fn init_bus(&mut self) {
        // Connect the CPU's read_byte and write_byte functions to the bus
        let read_byte_fn = Arc::new(Mutex::new(Box::new(|address: u16| {
            self.bus.read_byte(address)
        }) as Box<dyn FnMut(u16) -> u8 + Send + 'static>));
        let write_byte_fn = Arc::new(Mutex::new(Box::new(|address: u16, value: u8| {
